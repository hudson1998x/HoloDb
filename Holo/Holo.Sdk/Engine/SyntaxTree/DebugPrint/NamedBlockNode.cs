...
/// </summary>
public partial class NamedBlockNode : DebugPrint
{
    /// <summary>
    ...
    public override void DebugPrint(StringBuilder builder, in ReadOnlySpan<char> source, int tabIndent = 0)
    {
        ...
        builder.AppendLine($"{indent}NamedBlockNode {{");
        Name.DebugPrint(builder, source, tabIndent + 1);

        builder.AppendLine($"{indent}    Fields {{");
        foreach (var t in Fields.Nodes)
        {
            t.DebugPrint(builder, source, tabIndent + 2);
        }
